...
```text
npx louisgo init -> 自然对话开发 -> $start 按需深度恢复 -> $finish 正式交接
```

| 阶段          | 用户做什么          | AI / CLI 做什么                                                                |
| ------------- | ------------------- | ------------------------------------------------------------------------------ |
| 启用项目      | `npx louisgo init`  | 创建 `.louisgo/`，写入协议文件、验证脚本、Codex skills 和 `AGENTS.md` 指令块。 |
...
`context`、`stats`、`confirm`、`log`、`status`、`verify`、`pause`、`handoff promote`、`codex setup` 是 AI、高级用户和兼容场景的工具，不是日常主路径。完整命令形式是 `louisgo context`、`louisgo stats`、`louisgo confirm`、`louisgo log`、`louisgo status`、`louisgo verify`、`louisgo pause`、`louisgo handoff promote` 和 `louisgo codex setup`。
...
| `.louisgo/ROADMAP.md`         | 需要跨会话追踪的稳定任务。                                            |
