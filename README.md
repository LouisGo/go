...
`context`、`confirm`、`log`、`status`、`verify`、`pause`、`handoff promote`、`codex setup` 是 AI、高级用户和兼容场景的工具，不是日常主路径。完整命令形式是 `louisgo context`、`louisgo confirm`、`louisgo log`、`louisgo status`、`louisgo verify`、`louisgo pause`、`louisgo handoff promote` 和 `louisgo codex setup`。
