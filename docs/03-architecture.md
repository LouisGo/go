...
4. 创建必需模板文件。
5. 创建 `verify.sh` 和 `verify.ps1`。
6. 不覆盖已有文件。
7. 输出后续建议，例如运行 `louisgo status`。
...
4. 读取 `BLOCKER.md`。
5. 读取 `QUICK_SAVE.md`。
6. 生成 `HANDOFF_DRAFT.md`。
7. 不写入 `HANDOFF.md`。
8. 提示用户 review 后执行 `louisgo handoff promote`。
