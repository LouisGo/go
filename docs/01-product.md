...
| `louisgo clear`          | 当前项目不再使用 LouisGo，或需要重置协议时  | 明确确认后删除项目 `.louisgo/`，并移除项目 agent 指令管理块。     |
...
需要移除当前项目 LouisGo 时，先运行 `louisgo clear --dry-run` 查看目标；实际删除运行 `louisgo clear`，并在终端中通过方向键选择列表确认。该命令只清理当前 Git 项目的 `.louisgo/` 和项目 agent 指令文件中 LouisGo 管理的指令块，不清理全局 Codex 配置或全局 skills。
